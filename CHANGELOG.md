...
- Testé et abandonné : connexion MCP native directe (`streamable-http`) entre Open WebUI (Docker) et `qgis-mcp-server`. Solution retenue : passer par `mcpo` (proxy MCP → OpenAPI) dans tous les cas, local ou mutualisé — pas seulement pour l'instance mutualisée comme supposé initialement.
- Test compte non-admin réussi : après activation du toggle admin **Direct Connections** et de la permission de groupe **Serveur d'outils directs** (**Intégrations** → **Gérer les serveurs d'outils**, pas **Connexions**), un utilisateur test non-admin a pu ajouter la connexion `mcpo` et obtenir une réponse réelle du modèle (infos des couches du projet QGIS ouvert) — valide le point #1 de la checklist de déploiement mutualisé.
