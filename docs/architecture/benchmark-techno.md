...
La colonne « Déploiement » ne relève que le runtime et le mode de distribution propres à chaque candidat ; les modalités de raccordement à Open WebUI (passerelle éventuelle, chemin de connexion, adressage) sont un choix d'implémentation et relèvent de `decisions.md`.

| Candidat | Pérennité | Dépendance à Excel | Déploiement | Capacités | Mode hybride | Vigilance |
|---|---|---|---|---|---|---|
| **[haris-musa/excel-mcp-server](https://github.com/haris-musa/excel-mcp-server)** | 3,8k ★, 416 forks, développement actif (8 releases, v0.1.8 le 12/04/2026) | Aucune | Serveur Python (openpyxl), lancé en stdio via `uvx`, `pip` ou `uv tool`. Seul candidat à proposer, en plus du mode stdio, un mode HTTP (`streamable-http`) qu'Open WebUI peut interroger directement, sans passerelle. En contrepartie, ce mode impose de déclarer un dossier racine (`EXCEL_FILES_PATH`) : tous les chemins demandés sont relatifs à ce dossier, et rien au-dessus n'est accessible. | Classeurs et feuilles, cellules et formules (syntaxe validée), mise en forme complète (police, couleurs, bordures, fusion, format numérique, mise en forme conditionnelle), graphiques (5 types), tableaux croisés dynamiques, tableaux structurés, insertion/suppression de lignes et colonnes. Validation de données en lecture seule ; pas de VBA ni de Power Query/DAX. | Non : écrit directement le fichier `.xlsx`, donc échoue si le fichier est ouvert dans Excel. | Aucune pour un usage batch ; à écarter si le mode hybride est requis. |
| **[negokaz/excel-mcp-server](https://github.com/negokaz/excel-mcp-server)** | 953 ★, 113 forks, développement à l'arrêt (dernière release le 19/07/2025) | Aucune en mode fichier ; Windows + Excel requis pour le mode « Live editing » | Serveur Node/Go multiplateforme, lancé en stdio via `npx`. Le mode « Live editing » impose en outre d'être sur le même poste Windows que la session Excel (COM/OLE local). | 7 outils : lister les feuilles, lire valeurs et formules avec pagination (utile sur les gros fichiers), écrire valeurs et formules, créer une feuille ou un tableau, copier une feuille, mettre en forme une plage. Pas de graphiques, ni tableaux croisés dynamiques, ni mise en forme conditionnelle, ni validation de données, ni VBA (un fork `vKenjo` les ajoute, non fusionné, 2 ★). | Oui en principe, via un backend COM dédié (`excel_ole.go`) qui pilote Excel au lieu d'écrire le fichier. Mentionné au README seulement, aucun retour d'usage trouvé. | Capacités nettement plus limitées que haris-musa pour l'usage courant ; le mode hybride est la seule vraie raison de le considérer, et reste à valider en pratique. |
| **[guillehr2/Excel-MCP-Server-Master](https://github.com/guillehr2/Excel-MCP-Server-Master)** | 33 ★, 16 forks, aucune release taguée, mainteneur unique | Aucune | Node, lancé en stdio via `npx`, mais installe au premier lancement des dépendances Python (fastmcp, openpyxl, pandas, numpy, matplotlib, xlsxwriter, xlrd, xlwt) : Node.js **et** Python requis, soit plus de composants à maintenir que haris-musa (Python seul). | Classeurs et feuilles, écriture de cellules et formules, tableaux avec mise en forme automatique, graphiques (50+ styles), tableaux de bord combinant plusieurs graphiques, filtrage, import/export CSV/JSON/SQL/PDF (seul candidat à le proposer). Tableaux croisés dynamiques annoncés dans la description du projet mais aucun outil dédié dans la documentation. Mise en forme conditionnelle non identifiée. | Non : même limite que haris-musa (écriture directe du fichier). | Écart entre fonctionnalités annoncées et outils réellement documentés (TCD) ; traction très faible et mainteneur unique. |
| **[sbroenne/mcp-server-excel](https://github.com/sbroenne/mcp-server-excel)** | 185 ★, 32 forks, développement très actif (412 commits, 127 releases, v1.8.68 le 28/05/2026) | Oui : Windows + Excel 2016 ou supérieur installé (pilotage via l'API COM) | Exécutable natif `mcp-excel.exe` en stdio, à ajouter au PATH ; également distribué en extension VS Code et en paquet `.mcpb`. Ni Python ni Node requis. Une icône en barre système permet de suivre les sessions actives. | Le plus complet des quatre : classeurs et feuilles (dont déplacement entre classeurs), plages (valeurs, formules, mise en forme, validation, protection), tableaux, tableaux croisés dynamiques avec champs calculés, graphiques (séries, tendances, étiquettes), mise en forme conditionnelle, macros VBA, Power Query et DAX/modèle de données (seul candidat à couvrir ce point), segments, plages nommées, connexions OLEDB/ODBC. | Oui, mode « Agent Mode » conçu pour cela : sur demande, la fenêtre Excel pilotée par l'outil est rendue visible et l'utilisateur suit les opérations en direct. Nuance : l'outil requiert l'accès exclusif au classeur, ce n'est donc pas une prise de main sur une session déjà ouverte par l'utilisateur. | Stabilité de l'automatisation COM à tester, le projet documentant lui-même deux rapports de bugs à ce sujet. Périmètre large à filtrer si l'Audiar juge le risque trop élevé : VBA (bloqué par défaut par Excel via « Trust access to the VBA project object model », réglage à vérifier sur les postes), connexions OLEDB/ODBC et Power Query, qui peuvent atteindre toute source pour laquelle un pilote est installé sur le poste — un filtrage éventuel ne pouvant porter que sur un outil entier, pas sur les fonctions internes. Les options `formatMCode`/`formatDax`, désactivées par défaut, envoient du code M ou DAX à des services tiers (`powerqueryformatter.com`, `daxformatter.com`) : à ne jamais activer. |
...
`sbroenne` ressort comme le candidat le plus complet sur l'ensemble des besoins (formules toujours justes car évaluées par le vrai moteur Excel, TCD, mise en forme conditionnelle, charte graphique via les styles natifs) et le seul à offrir un vrai mode hybride ("Agent Mode"), avec une distribution plus simple à généraliser sur les postes (exécutable autonome sans runtime Python ni Node à installer).

Cependant, la fiabilité de l'automatisation COM en usage réel reste à tester ; son périmètre large (VBA, connexions OLEDB/ODBC, Power Query) devrait être filtré si l'Audiar estime le risque trop important.
